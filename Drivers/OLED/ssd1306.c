...
void SSD1306_Invert(int x0, int y0, int x1, int y1)
{
		for (int i = 0; i < sizeof(SSD1306_Buffer); i++)
		{
			for(int y = y0/8; y <= y1/8; y++)
			{
				if(y==0||y==2||y==3||y==5)
				{
					if(i>=x0+y*SSD1306_WIDTH&&i<=x1+y*SSD1306_WIDTH)
						SSD1306_Buffer[i] = ~SSD1306_Buffer[i];
				}
				else if((y==1&&y0==0)||(y==4&&y0==24))
				{
					if(i>=x0+y*SSD1306_WIDTH&&i<=x1+y*SSD1306_WIDTH)
					{
						uint8_t temp = (~SSD1306_Buffer[i]) & 0x0f;
						SSD1306_Buffer[i] &= 0xf0;
						SSD1306_Buffer[i] += temp;
					}
				}
				else
				{
					if(i>=x0+y*SSD1306_WIDTH&&i<=x1+y*SSD1306_WIDTH)
					{
						uint8_t temp = (~SSD1306_Buffer[i]) & 0xf0;
